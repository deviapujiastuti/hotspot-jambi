...
/* ── HERO ── */
.hero-wrap {
    background: linear-gradient(135deg, rgba(255,69,0,0.15) 0%, rgba(255,140,0,0.08) 40%, rgba(139,0,0,0.08) 100%);
    ...
    position: relative;
    overflow: hidden;
    box-shadow: 0 8px 32px rgba(255,69,0,0.15), inset 0 1px 0 rgba(255,255,255,0.1);
...
.hero-sub b { color: #FFB366; font-weight: 600; }
...
.metric-card {
    background: linear-gradient(135deg, rgba(255,69,0,0.08) 0%, rgba(255,140,0,0.04) 100%);
...
.info-box b { color: #FFB366; font-weight: 700; }
...
/* ── DBSCAN TABLE ── */
.dbscan-table { width: 100%; border-collapse: collapse; font-size: 0.9rem; margin-top: 0.5rem; }
.dbscan-table th {
    background: linear-gradient(90deg, rgba(255,69,0,0.15) 0%, rgba(255,140,0,0.08) 100%);
    color: #FFB366;
    font-weight: 700;
    font-size: 0.8rem;
    text-transform: uppercase;
...
.footer-wrap b { color: #FFB366; font-weight: 700; }
</style>

<!-- BURGER BUTTON & OVERLAY -->
<button class="burger-btn" onclick="toggleSidebar()" id="burgerBtn" aria-label="Buka menu">
    <span></span>
    <span></span>
    <span></span>
</button>
...
function toggleSidebar() {
    ...
    if (!sidebar) {
        // fallback: cari di document biasa
...
// Auto close sidebar saat klik di luar (mobile)
...
<div class="hero-wrap">
    ...
    <div class="hero-title">Pemetaan Spasio-Temporal<br>Hotspot Karhutla Jambi</div>
    <p class="hero-sub">
...
with col_map:
    ...
    if len(df_f) == 0:
        ...
        m = folium.Map(location=[-1.5, 102.8], zoom_start=7, tiles="CartoDB dark_matter")
        folium.Rectangle(
            bounds=[[-3.0, 101.5], [0.0, 104.7]],
            color="#FF4500", fill=False, weight=2,
            dash_array="8 5", tooltip="Batas Administratif Jambi"
        ).add_to(m)

        if mode_peta == "Heatmap":
            heat_data = df_f[['latitude', 'longitude', 'frp']].values.tolist()
            HeatMap(heat_data, min_opacity=0.3, max_zoom=10, radius=15, blur=18,
                    gradient={0.2:'#0066ff', 0.4:'#00ff00', 0.6:'#ffff00', 0.8:'#ff6600', 1.0:'#ff0000'}).add_to(m)
            legend_html = """
            <div style="position:fixed;bottom:20px;left:20px;z-index:1000;
                ...
                <div style="color:#FF6B35;font-weight:700;letter-spacing:.05em;margin-bottom:10px;">INTENSITAS FRP</div>
                <div style="font-size:11px;">
                    <div style="margin-bottom:5px;"><span style="color:#0066ff;font-weight:600;">■</span>&nbsp;Rendah</div>
                    <div style="margin-bottom:5px;"><span style="color:#00ff00;font-weight:600;">■</span>&nbsp;Sedang</div>
                    <div style="margin-bottom:5px;"><span style="color:#ffff00;font-weight:600;">■</span>&nbsp;Tinggi</div>
                    <div style="margin-bottom:5px;"><span style="color:#ff6600;font-weight:600;">■</span>&nbsp;Sangat Tinggi</div>
                    <div><span style="color:#ff0000;font-weight:600;">■</span>&nbsp;Ekstrem</div>
                </div>
            </div>"""
            ...
            for _, row in sample.iterrows():
                folium.CircleMarker(
                    ...
                    fill_color='#FF6B35', fill_opacity=0.8, weight=1.5,
                    popup=folium.Popup(
                        f"""<div style='font-family:Poppins,sans-serif;font-size:12px;min-width:200px;color:#333;'>
                            <div style='background:linear-gradient(135deg,#FF4500,#FFB366);color:white;
                                        padding:8px;border-radius:6px;font-weight:700;margin-bottom:8px;'>
                                📍 {row['nama_daerah']}
                            </div>
            ...
            for _, row in df_f.iterrows():
                if row['cluster'] == -1:
                    color, label, radius = '#555566', 'Noise/Outlier', 2.5
                elif row['cluster'] in top_clusters:
                    color = clr[top_clusters.index(row['cluster']) % len(clr)]
                    label, radius = f"Kluster {row['cluster']}", 4
                else:
                    color, label, radius = '#FFAA00', f"Kluster {row['cluster']}", 3
                folium.CircleMarker(
                    location=[row['latitude'], row['longitude']],
                    ...
                    popup=folium.Popup(
                        f"""<div style='font-family:Poppins,sans-serif;font-size:12px;min-width:200px;color:#333;'>
                            <div style='background:linear-gradient(135deg,{color},#FFB366);color:white;
                                        padding:8px;border-radius:6px;font-weight:700;margin-bottom:8px;'>
                                📍 {row['nama_daerah']}
                            </div>
                            <table style='width:100%;border-collapse:collapse;font-size:11px;'>
            ...
            <div style="position:fixed;bottom:20px;left:20px;z-index:1000;
                        ...
                        border:2px solid rgba(255,69,0,0.3);font-family:Poppins,sans-serif;
                        box-shadow:0 8px 25px rgba(255,69,0,0.2);">
                <div style="color:#FF6B35;font-weight:700;letter-spacing:.05em;margin-bottom:10px;">KLUSTER DBSCAN</div>
                <div style="font-size:11px;">
                    <div style="margin-bottom:5px;"><span style="color:#e41a1c;font-weight:600;">●</span>&nbsp;Top 12 Kluster</div>
                    <div style="margin-bottom:5px;"><span style="color:#ffaa00;font-weight:600;">●</span>&nbsp;Kluster Lainnya</div>
                    <div><span style="color:#555566;font-weight:600;">●</span>&nbsp;Noise/Outlier</div>
                </div>
            </div>"""
            m.get_root().html.add_child(folium.Element(legend_html))
...
with col_chart:
    ...
    if len(df_f) > 0:
        ...
        fig_tahun = px.bar(
            df_tahun_f, x='year', y='count', color='count',
            color_continuous_scale=[[0,'#0066ff'],[0.25,'#00ff00'],[0.5,'#ffff00'],[0.75,'#ff6600'],[1,'#ff0000']],
            labels={'year':'Tahun','count':'Hotspot'}, title='Hotspot per Tahun'
        )
        ...
        fig_bulan = px.area(
            ...
            labels={'month':'Bulan','count':'Hotspot'}, title='Distribusi per Bulan',
            color_discrete_sequence=['#FF4500']
        )
        fig_bulan.update_traces(
            fill='tozeroy', fillcolor='rgba(255,69,0,0.25)',
...
with col_a:
    if len(df_kluster) > 0:
        ...
        fig_cluster = px.bar(
            top10, x='nama_daerah', y='jumlah_titik', color='avg_frp',
        ...
        fig_cluster.update_layout(
            ...
            font=dict(color='#9a9aaa', family='Poppins', size=11),
            title=dict(font=dict(size=13, color='#e8e8e8'), x=0.05),
            height=340, margin=dict(t=40, b=100, l=10, r=10),
            coloraxis_colorbar=dict(tickfont=dict(color='#9a9aaa')),
...
with col_b:
    if len(df_kluster) > 0:
        ...
        avg_frp_tb    = df_kluster.loc[df_kluster['jumlah_titik'].idxmax(), 'avg_frp']

        st.markdown(f"""
        <table class="dbscan-table">
            <tr><th style="width:45%;">Metrik</th><th>Nilai</th></tr>
            <tr><td>🤖 Algoritma</td><td><span class="badge-algo">DBSCAN</span></td></tr>
            <tr><td>📏 Epsilon</td><td><b style="color:#FF6B35;">0.05°</b> (~5.5 km)</td></tr>
            <tr><td>🎯 Min Samples</td><td><b style="color:#FF6B35;">5 titik</b></td></tr>
            <tr><td>📊 Total Kluster</td><td><b style="color:#FFB366;">{n_kluster}</b></td></tr>
            <tr><td>🔸 Titik Noise</td><td><b style="color:#ffffff;">{n_noise:,}</b></td></tr>
            <tr><td>🔥 Kluster Terbesar</td><td><b style="color:#FF6B35;">{n_terbesar:,}</b> titik</td></tr>
            <tr><td>📍 Zona Rawan Utama</td><td><b style="color:#FFB366;">{nama_terbesar}</b></td></tr>
            <tr><td>⚡ Avg FRP Utama</td><td><b style="color:#FF6B35;">{avg_frp_tb:.2f}</b> MW</td></tr>
        </table>
        """, unsafe_allow_html=True)

        st.markdown("""
        <div style="background:linear-gradient(135deg,rgba(255,69,0,0.12) 0%,rgba(255,140,0,0.06) 100%);
                    border:2px solid rgba(255,69,0,0.2);border-left:4px solid #FF4500;
                    border-radius:10px;padding:1rem 1.2rem;margin-top:1rem;
                    font-size:0.85rem;color:#b8b8cc;line-height:1.6;">
            <b style="color:#FF6B35;">💡 Interpretasi:</b><br>
            Kluster dengan jumlah titik besar dan FRP tinggi menunjukkan zona konsentrasi 
            karhutla dengan risiko tinggi. Prioritaskan mitigasi dan pemantauan di area tersebut.
        </div>
        """, unsafe_allow_html=True)
...
# ── DATA MENTAH ───────────────────────────────────────────────
st.markdown('<div class="custom-divider"></div>', unsafe_allow_html=True)
with st.expander("📄 Eksplorasi Data Mentah (sample 500 baris)", expanded=False):
    if len(df_f) > 0:
        st.dataframe(
            df_f.sample(min(500, len(df_f)), random_state=42)[[
                'acq_date','latitude','longitude','brightness','frp',
                'confidence','year','month_name','cluster_label','nama_daerah'
